...
class SphinxSearch extends SpecialPage {
	...
	function execute( $par ) {
		...
		} elseif ($cl == false && trim ($term) == "") {
			// nothing returned from the client, but nothing was searched for
		} else {
	...
	function wfSphinxDisplayResults( $term, $res, $cl ) {
		...
		if ( isset( $res["matches"] ) && is_array( $res["matches"] ) ) {
			$wgOut->addWikiText( "----" );
			$excerpts_opt = array(
				"before_match"    => "<span style='font-weight: bold;'>",
				"after_match"     => "</span>",
				"chunk_separator" => " ... ",
				"limit"           => 100,
				"around"          => 5
			...
			foreach ( $res["matches"] as $doc => $docinfo ) {
				$l = $wgSphinxMultiLanguages[$docinfo['attrs']['lang']];
				$old_id = $docinfo['attrs']['old_id'];
				$page_content = $this->sphinxMultiSelectField(
					'text', 'old_text',
					array(
						'old_id' => $old_id
					),
				...
				if ( $page_content ) {
					//$title_obj = Title::newFromID( $docId );
					$title_obj = $this->sphinxMultiNewFromID($doc, $docinfo['attrs']['lang']);
					if ( is_object( $title_obj ) ) {
						$wiki_title = str_replace($term, "'''{$term}'''", $title_obj->getPrefixedText());
						$wiki_path = $title_obj->getPrefixedDBkey();
						
						#$wiki_path = str_replace('$1', $wiki_path, $l['wiki_url']);
						//$wgOut->addWikiText( "* <span style='font-size:110%;'>[[:$wiki_path|$wiki_title]]</span>" );
						$wgOut->addWikiText( "* <span style='font-size:110%;'>[[:{$l['lang']}:{$wiki_path}|{$wiki_title}]]</span>" );
						...
						foreach ( $excerpts as $entry ) {
							...
							$wgOut->addHTML( "<div style='margin: 0.2em 1em 1em 1em; width:450px'>$entry</div>\n" );
						}
					}
				}
			}
	...
	function sphinxMultiSelectField($table, $var, $cond='', $fname = 'Database::selectField' , $langId = null) {
		global $wgSphinxMultiMiltiplier, $wgSphinxMultiLanguages;
		
		$dbr = wfGetDB( DB_SLAVE );
		$l = $wgSphinxMultiLanguages[$langId];
		$t = $l['db']. '.' . $l['prefix'] .$table;
		return $dbr->selectField($t, $var, $cond, $fname);
	}
	
	function sphinxMultiNewFromID( $docId, $langId) {
		global $wgSphinxMultiMiltiplier, $wgSphinxMultiLanguages;
		$l = $wgSphinxMultiLanguages[$langId];
		
		$id = ($docId - $langId) / $wgSphinxMultiMiltiplier;
		$db = ($flags & GAID_FOR_UPDATE) ? wfGetDB( DB_MASTER ) : wfGetDB( DB_SLAVE );
		
		$row = $db->selectRow( $l['db']. '.' . $l['prefix'] . 'page', '*', array( 'page_id' => $id ), __METHOD__ );
		if( $row !== false ) {
			$title = $this->sphinxMultiNewFromRow( $row );
		} else {
			$title = null;
		}
		return $title;
	 ...
	 */
	function sphinxMultiNewFromRow( $row ) {
